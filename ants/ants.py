...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                assert (self.ant.is_container or insect.is_container) and not (self.ant.is_container and insect.is_container), 'Two ants in {0}'.format(self)
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(insect, QueenAnt) and insect.armor > 0 and insect.is_true:
                return 
            # END Problem 13
    ...
    def nearest_bee(self, hive):
        ...
        if self.min_range != None:
            for _ in range(self.min_range):
                p = p.entrance
        while p.bees == [] and p.entrance != None:
            p = p.entrance
            i += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.armor -= amount
        i, length = 0, len(self.place.bees)
        if self.armor <= 0:
            while i < length:
    ...
    def action(self, colony):
        ...
        if not self.is_true:
            ...
            while p.exit != None:
                p = p.exit
                if p.ant != None:
                    if not p.ant.is_buffed:
                        p.ant.is_buffed = True
                        p.ant.damage *= 2
                    if p.ant.is_container and p.ant.contained_ant != None:
                        if not p.ant.contained_ant.is_buffed:
